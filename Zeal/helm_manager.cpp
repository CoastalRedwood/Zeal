...
// For race/gender combinations that have bugged Velious helms, we've patch in a head (4) material that we use whenever
// they would normally show the default (0) head. If the patch installed, we used material (4) whenever they are showing
// the default head, rather than material (0).
...
int __fastcall EntityChangeForm_hk(Zeal::GameStructures::Entity *entity, int unused_edx,
                                   Zeal::Packets::Illusion_Struct *illusion) {
...
// client had buggy logic that was not able to correctly hide some races' hair from clipping through the helmets. Or
// when it did manage to hide the hair from clipping, it also hides the hair on all characters with the same face in the
// zone, even if they didn't have helms on, making their heads become see-through.
//
...
// - We also have a custom s3d where the base head (material 0) is patched to not have hair at all. Instead we use
...
int HelmManager::HandleSwapHead(Zeal::GameStructures::Display *cDisplay, Zeal::GameStructures::Entity *entity,
  ...
  if (entity == Zeal::Game::get_self()) {
    if (HELM_MANAGER_LOG_DEBUG) {
      Zeal::Game::print_chat("[HelmManager] [SwapHead_hk] RECV SwapHead (%i -> %i) %s Race:%u Gender:%u", old_material,
                             new_material, entity->Name, entity->Race, entity->Gender);
    }
    if (!zeal->helm->ShowHelmEnabled.get() && Zeal::Game::IsPlayableRace(entity->Race)) {
  ...
  if (zeal->helm->IsHelmBuggedOldModel(entity->Race, entity->Gender)) {
    ...
    // - Convert the default Head Material (0) to a custom Head Material (4). We use (4) as the default head for these
    // bugged races.
    ...
    // Game passes in the Item->Material/IDFile value (e.g. '0'), which is inconsistent with the head state when we're
    // using head overrides, so we have to correct for this,
  ...
  // - When equipping a non-Velious Helm when local_only=false, this causes a double-send with a wrong value in the
  // first packet. Suppressing this causes it to single-send the correct value only.
  // - But we don't want to suppress Velious Helms, as those do no exhibit the double-send bug, and it would instead
  // cause nothing to be sent.
...
#pragma warning(suppress : 6237)
  ...
  int result = zeal->hooks->hook_map[SwapHeadHook]->original(SwapHead_hk)(cDisplay, 0, entity, new_material,
                                                                          old_material, color, local_only);

  // (4) Fix CDisplay::SwapHead() overflow:
  // The original call only writes the lo-byte of the 'new_material' value to entity->EquipmentMaterialType[0] in some
  // code paths. Because of our new logic, we can overflow this when custom IT### numbers are introduced (in the
  // 500-600). Simply overwriting this value after the call works fine.
...
int HelmManager::HandleIllusion(Zeal::GameStructures::Entity *entity, Zeal::Packets::Illusion_Struct *is) {
  if (HELM_MANAGER_LOG_DEBUG) {
    Zeal::Game::print_chat("[HelmManager] [Illusion] RECV race=%i gender=%u head=%u (hi=%u)", is->race, is->gender,
                           is->helmtexture, is->unknown007);
  }
  ...
  WORD color = 0;
  WORD old_material =
      entity->EquipmentMaterialType[kMaterialSlotHead];  // '4' or some Velious ID are the ones we need to clean up
  ...
  if (Zeal::Game::IsPlayableRace(is->race)) {
  ...
  if (set_helm_material_before_illusion != 0xFFFF) {
    if (HELM_MANAGER_LOG_DEBUG) {
      Zeal::Game::print_chat("[HelmManager] [Illusion] PRE-ILLUSION SwapHead (%i -> %i) %s Race:%u Gender:%u",
                             old_material, set_helm_material_before_illusion, entity->Name, entity->Race,
                             entity->Gender);
    }
    // Calling directly to the original function here because we want to bypass our Velious-fix logic, to actually set
    // their material to potentially 0 (not 4).
  ...
  int result =
      ZealService::get_instance()->hooks->hook_map["EntityChangeForm"]->original(EntityChangeForm_hk)(entity, 0, is);
  ...
  if (refresh_helm) {
    ...
    if (HELM_MANAGER_LOG_DEBUG) {
      Zeal::Game::print_chat("[HelmManager] [Illusion] POST-ILLUSION SwapHead (%i -> %i) %s Race:%u Gender:%u",
                             kMaterialNone, set_helm_material_after_illusion, entity->Name, entity->Race,
                             entity->Gender);
    }
    ...
    if (new_material != kMaterialNone && !ShowHelmEnabled.get()) {
      if (HELM_MANAGER_LOG_DEBUG) {
        Zeal::Game::print_chat(
            "[HelmManager] [WearChange] Got inbound helm while our helm is hidden, hiding our helm.");
      }
      RefreshHelm(false, 0);
    }
    // Inbound WearChanges from the server shouldn't generate a response. However, the client doesn't respect
    // 'local_only' flag on some helmets.
    block_wearchange = 1;
  }
  block_outbound_wearchange += block_wearchange;
  ZealService::get_instance()->hooks->hook_map["WearChangeArmor"]->original(WearChangeArmor_hk)(
      cDisplay, 0, spawn, wear_slot, new_material, old_material, colors, local_only);
  block_outbound_wearchange -= block_wearchange;
}
...
bool HelmManager::Handle_In_OP_WearChange(Zeal::Packets::WearChange_Struct *wc) {
  if (wc && wc->wear_slot_id == kMaterialSlotHead) {
    ...
    if (HELM_MANAGER_LOG_DEBUG) {
      Zeal::GameStructures::Entity *spawn =
          (self && self->SpawnId == wc->spawn_id) ? self : Zeal::Game::get_entity_by_id(wc->spawn_id);
      Zeal::Game::print_chat("[HelmManager] [OP_WearChange] RECV WearChange from %s Material=%u Color=0x%08x",
                             spawn ? spawn->Name : "Unknown", wc->material, wc->color.Color);
    }
  }
...
bool HelmManager::Handle_Out_OP_WearChange(Zeal::Packets::WearChange_Struct *wc) {
  if (wc && wc->wear_slot_id == kMaterialSlotHead) {
    ...
    if (self) {
      ...
      last_wc = *wc;
      if (HELM_MANAGER_LOG_DEBUG) {
        Zeal::Game::print_chat("[HelmManager] [OP_WearChange] SEND WearChange Material=%u Color=0x%08x", wc->material,
                               wc->color.Color);
      }
    }
  }
...
// - [5xx/6xx] -> [240] Swaps our racial Velious head model IT### back to the generic '240' value used by all Velious
// helms.
// - etc..
WORD HelmManager::ToCanonicalHelmID(WORD material, WORD race) {
  if (Zeal::Game::IsPlayableRace(race)) {
    switch (material) {
      // Converts our custom Zeal/Server Head ID back to the normal default head
...
//   But the bugged races we must set them via their racial IT### number to bypass the helm from triggering the hair
//   transparency toggles. If we don't do it, then hair becomes transparent for people without the helm on.
WORD HelmManager::ToRacialHelmMaterial(WORD material, WORD race, BYTE gender) {
  ...
  if (material == kMaterialVeliousHelmOgreRZ) {
    if (race == 10) {
      return kMaterialVeliousHelmOgreRZ;
    } else if (Zeal::Game::IsPlayableRace(race)) {
...
bool HelmManager::IsHelmBuggedOldModel(WORD race, BYTE gender) {
  if (!DetectInstalledFixesComplete) {
    DetectInstalledFixes();  // Have to lazy-loid this here because this is reached before OnZone() is called during
                             // initial login.
  }
...
WORD HelmManager::GetHelmMaterial(Zeal::GameStructures::GAMEITEMINFO *item) {
  if (!item || item->Type != 0) {
    return 0;
  }
...
void HelmManager::RefreshHelm(bool local_only, int delay_ms) {
  if (!refresh_helm_pending) {
    refresh_helm_pending = true;
    ZealService::get_instance()->callbacks->AddDelayed([this, local_only]() { RefreshHelmCallback(local_only); },
                                                       delay_ms);
  }
}
...
void HelmManager::RefreshHelmCallback(bool local_only) {
  ...
  if (self && char_info && refresh_helm_pending) {
    ...
    if (Zeal::Game::IsPlayableRace(self->Race)) {
      ...
      if (ShowHelmEnabled.get() && char_info->Inventory.Head) {
        ...
        if (HELM_MANAGER_LOG_DEBUG) {
          Zeal::Game::print_chat("[HelmManager] [UpdateHelm] Current helm is %u color %u", material, color);
        }
      }
      ...
      if (!local_only) {
        if (HELM_MANAGER_LOG_DEBUG) {
          Zeal::Game::print_chat("[HelmManager] [UpdateHelm] Manually sending WearChange %u color %u", material, color);
        }
        ...
        if (self->SpawnId != last_wc.spawn_id || material != last_wc.material || color != last_wc.color.Color) {
          ...
          Zeal::Game::send_message(Zeal::Packets::WearChange, (int *)&wc, sizeof(Zeal::Packets::WearChange_Struct), 0);
        } else if (HELM_MANAGER_LOG_DEBUG) {
          Zeal::Game::print_chat(
              "[HelmManager] [UpdateHelm] Skipping sending WearCchange, we already are in sync with server.");
        }
      }
    }
  }
...
void HelmManager::OnZone() {
  ...
  if (!char_info || !Zeal::Game::is_in_game()) {
    // Wait and try again
...
void HelmManager::SyncShowHelm(bool server_silent) {
  Zeal::Game::do_say(true, "#showhelm %s%s", ShowHelmEnabled.get() ? "on" : "off", server_silent ? " silent" : "");
}
      ...
      if (fn) {
        ...
        if (initialized) {
          ...
          UseBarbarianFemaleFix = !ini.getValue<bool>("Defaults", "UseLuclinBarbarianFemale") &&
                                  t3dGetPointerFromDictionary(dictionary, "BAFHE04_DMSPRITEDEF");
          UseEruditeFemaleFix = !ini.getValue<bool>("Defaults", "UseLuclinEruditeFemale") &&
                                t3dGetPointerFromDictionary(dictionary, "ERFHE04_DMSPRITEDEF");
          UseEruditeMaleFix = !ini.getValue<bool>("Defaults", "UseLuclinEruditeMale") &&
                              t3dGetPointerFromDictionary(dictionary, "ERMHE04_DMSPRITEDEF");
          UseWoodElfFemaleFix = !ini.getValue<bool>("Defaults", "UseLuclinWoodElfFemale") &&
                                t3dGetPointerFromDictionary(dictionary, "ELFHE04_DMSPRITEDEF");
          UseDarkElfFemaleFix = !ini.getValue<bool>("Defaults", "UseLuclinDarkElfFemale") &&
                                t3dGetPointerFromDictionary(dictionary, "DAFHE04_DMSPRITEDEF");
          if (HELM_MANAGER_LOG_DEBUG) {
            Zeal::Game::print_chat("[HelmManager] [DetectInstalledFixes] Complete");
          }
  ...
  if (HELM_MANAGER_LOG_DEBUG) {
    zeal->commands_hook->Add("/helmconfig", {}, "Prints/Toggles HelmManager configuration (for testing).",
                             [this](std::vector<std::string> &args) {
                               ...
                               Zeal::Game::print_chat("[1] ShowHelm: %s", ShowHelmEnabled.get() ? "on" : "off");
                               Zeal::Game::print_chat("[2] Fix Human (F): %s", UseHumanFemaleFix ? "on" : "off");
                               Zeal::Game::print_chat("[3] Fix Barbarian (F): %s",
                                                      UseBarbarianFemaleFix ? "on" : "off");
                               Zeal::Game::print_chat("[4] Fix Erudite (F): %s", UseEruditeFemaleFix ? "on" : "off");
                               Zeal::Game::print_chat("[5] Fix Erudite (M): %s", UseEruditeMaleFix ? "on" : "off");
                               Zeal::Game::print_chat("[6] Fix WoodElf (F): %s", UseWoodElfFemaleFix ? "on" : "off");
                               Zeal::Game::print_chat("[7] Fix DarkElf (F): %s", UseDarkElfFemaleFix ? "on" : "off");
                               return true;
                             });
  }
}
