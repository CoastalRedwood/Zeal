 ...
 * ratio. */
/*  (This special func. is currently only enabled when MINIZ_USE_UNALIGNED_LOADS_AND_STORES and MINIZ_LITTLE_ENDIAN
 * are defined.) */
/* Return values: */
/*  MZ_OK on success. */
...
/*   MZ_STREAM_END if all input has been consumed and all output bytes have been written. Don't call mz_deflate() on
 * the stream anymore. */
/*   MZ_STREAM_ERROR if the stream is bogus. */
 ...
 * not the stream has been wrapped with a zlib header/footer: */
...
/*   MZ_FINISH implies that there are no more source bytes available beside what's already in the input buffer, and
 * that the output buffer is large enough to hold the rest of the decompressed data. */
/* Return values: */
 ...
 * input to continue, or if the output buffer is not large enough. Call mz_inflate() again */
...
/* TDEFL_NONDETERMINISTIC_PARSING_FLAG: Enable to decrease the compressor's initialization time to the minimum, but
 * the output may vary from run to run given the same input (depending on the contents of memory). */
...
/*  The image pitch in bytes per scanline will be w*num_chans. The leftmost pixel on the top scanline is stored first
 * in memory. */
/*  level may range from [0,10], use MZ_NO_COMPRESSION, MZ_BEST_SPEED, MZ_BEST_COMPRESSION, etc. or a decent default
 * is MZ_DEFAULT_LEVEL */
 ...
 * the other functions are just high-level helpers for improved usability. */
...
typedef enum {
  ...
  MZ_ZIP_FLAG_WRITE_ZIP64 = 0x4000, /* always use the zip64 file format, instead of the original zip file format with
...
/* actual_archive_size is the true total size of the archive, which may be smaller than the file's actual size on
 * disk. If zero the entire file is treated as the archive. */
...
/* MZ_TRUE if the compression method is supported, and the file is not encrypted, and the file is not a compressed
 * patch file. */
 ...
 * existing archive. */
/* For archives opened using mz_zip_reader_init_file, pFilename must be the archive's filename so it can be reopened
 * for writing. If the file can't be reopened, mz_zip_reader_end() will be called. */
/* For archives opened using mz_zip_reader_init_mem, the memory block must be growable using the realloc callback
 * (which defaults to realloc unless you've overridden it). */
/* Finally, for archives opened using mz_zip_reader_init, the mz_zip_archive's user provided m_pWrite function cannot
 * be NULL. */
/* Note: In-place archive modification is not recommended unless you know what you're doing, because if execution
 * stops or something goes wrong before */
...
/* Adds the contents of a file to an archive. This function also records the disk file's modified time into the
 * archive. */
 ...
 * extra data (it may add or modify the zip64 local header extra data field), and the optional descriptor following
 * the compressed data. */
 ...
 */
/* Note for the archive to be valid, it *must* have been finalized before ending (this function will not do it for
 * you). */
