...
// - Interactive mode:
//   - Game client code appears to ignore the middle mouse button even though callbacks exist
...
//     happens to chat windows, so it's a game client issue.
...
//
// Coordinate systems:
//  - The game world coordinate system has "x" as up and "y" as left on the screen.
...
//    game world coordinates (Position, location values) are translated to it when
//    used by flipping and negating (polarity) the y and x values.
//    - Variables called "position" or "loc" are typically in game coordinates (unconverted)
...
void ZoneMap::render_update_viewport(IDirect3DDevice8 &device) {
  ...
  if (external_enabled) {
    viewport = D3DVIEWPORT8{
        .X = 0, .Y = 0, .Width = description.Width, .Height = description.Height, .MinZ = 0.0f, .MaxZ = 1.0f};
    return;
  }
  ...
  if (map_interactive_enabled) {
    ...
    if (rect.Left < -1)
      viewport.Width =
          max(kWinMinSize, min(max_viewport_width - static_cast<int>(viewport.X), rect.Right - rect.Left - 1));
    else
  ...
  if (!wnd->IsMinimized) Zeal::Game::GameInternal::CXWndGetClientRect(wnd, 0, &map_client_rect);
...
void ZoneMap::ui_synchronize_window() {
  if (external_enabled || !map_interactive_enabled) return;
  ...
  Zeal::Game::GameInternal::CXWndMoveAndInvalidate(wnd, 0, viewport.X - rect.Left, viewport.Y - rect.Top,
                                                   viewport.X + viewport.Width - rect.Right,
                                                   viewport.Y + viewport.Height - rect.Bottom);
}
...
void ZoneMap::render_update_transforms(const ZoneMapData &zone_map_data) {
  ...
  const Vec3 position = (show_zone_id == kInvalidZoneId)
                            ? Zeal::Game::get_self()->Position
                            : Vec3(-static_cast<float>(zone_map_data.min_y + zone_map_data.max_y) /
                                       2,  // Center on map, flipping y and x.
  ...
  const bool align_top = full_height || (position_y - zone_map_data.min_y) * scale_factor < viewport_height * 0.5f;
  ...
  const bool center_on_position_x = always_align_to_center || (!full_width && !align_left && !align_right);
  ...
  clip_min_x = max(static_cast<float>(zone_map_data.min_x), (-viewport_width * 0.5f - offset_x) / scale_factor);
  clip_max_x = min(static_cast<float>(zone_map_data.max_x), (+viewport_width * 0.5f - offset_x) / scale_factor);
  clip_min_y = max(static_cast<float>(zone_map_data.min_y), (-viewport_height * 0.5f - offset_y) / scale_factor);
  clip_max_y = min(static_cast<float>(zone_map_data.max_y), (+viewport_height * 0.5f - offset_y) / scale_factor);
}
...
D3DCOLOR ZoneMap::render_get_line_color_and_opacity(const ZoneMapLine &line, int position_z, int level_id) const {
  ...
  if ((!external_enabled || map_background_state == BackgroundType::kDark) && line.red == 0 && line.green == 0 &&
      line.blue == 0) {
...
void ZoneMap::render_load_map(IDirect3DDevice8 &device, const ZoneMapData &zone_map_data) {
  ...
  for (int i = 0; i < line_count; ++i) {
    ...
    line_vertices.push_back(
        {.x = static_cast<float>(line.x0), .y = static_cast<float>(line.y0), .z = 0.5f, .color = color});
    line_vertices.push_back(
        {.x = static_cast<float>(line.x1), .y = static_cast<float>(line.y1), .z = 0.5f, .color = color});
  }
  ...
  MapVertex background_vertices[kBackgroundVertices] = {
      {static_cast<float>(zone_map_data.min_x), static_cast<float>(zone_map_data.min_y), 0.5f,
       background_color},  // x, y, z,color
      {static_cast<float>(zone_map_data.max_x), static_cast<float>(zone_map_data.min_y), 0.5f, background_color},
      {static_cast<float>(zone_map_data.min_x), static_cast<float>(zone_map_data.max_y), 0.5f, background_color},
      {static_cast<float>(zone_map_data.max_x), static_cast<float>(zone_map_data.max_y), 0.5f, background_color},
  };
  ...
  std::string description = std::format("{0}: {1}", zone_map_data.name, Zeal::Game::get_full_zone_name(zone_id));
  set_window_title(description.c_str());
}

std::vector<ZoneMap::MapVertex> ZoneMap::calculate_grid_vertices(const ZoneMapData &zone_map_data) const {
  ...
  const int grid_x_min = ((zone_map_data.min_x + grid_x_ceil) / map_grid_pitch) * map_grid_pitch;
  ...
  const int grid_y_min = ((zone_map_data.min_y + grid_y_ceil) / map_grid_pitch) * map_grid_pitch;
  ...
  for (int i = 0; i < grid_x_count; ++i) {
    ...
    vertices.push_back({.x = x, .y = static_cast<float>(zone_map_data.min_y), .z = 0.5f, .color = color});
    vertices.push_back({.x = x, .y = static_cast<float>(zone_map_data.max_y), .z = 0.5f, .color = color});
  }
...
D3DCOLOR ZoneMap::get_background_color() const {
  ...
  unsigned int alpha = external_enabled ? 255 : static_cast<int>(map_background_alpha * 255 + 0.5f);
...
void ZoneMap::update_succor_label() {
  ...
  const auto *zone_info = Zeal::Game::ZoneInfo;
...
void ZoneMap::render_load_labels(IDirect3DDevice8 &device, const ZoneMapData &zone_map_data) {
  ...
  for (int i = 0; i < num_labels_to_scan; ++i) {
    ...
    if (label.z < clip_min_z || label.z > clip_max_z) continue;  // Not on visible level, skip it.
...
std::vector<std::string> ZoneMap::get_available_fonts() const { return BitmapFont::get_available_fonts(); }
...
void ZoneMap::render_map(IDirect3DDevice8 &device) {
  ...
  D3DXMatrixOrthoLH(&mat_project, static_cast<float>(viewport.Width), -static_cast<float>(viewport.Height), 0.f,
                    100.f);  // Z may need tweaking.
  ...
  if (map_background_state != BackgroundType::kClear && !external_enabled) render_background(device);

  if (map_show_grid) render_grid(device);
  ...
  if (show_zone_id == kInvalidZoneId) render_positions(device);
...
void ZoneMap::render_markers(IDirect3DDevice8 &device) {
  ...
  if (marker_vertex_buffer == nullptr) return;  // Failed to update above.
  ...
  for (const auto &marker : markers_list) {
    render_label_text(marker.label.c_str(), -marker.loc_y, -marker.loc_x, color, LabelType::PositionLabel,
                      offset_pixels);
  }
...
void ZoneMap::render_labels(IDirect3DDevice8 &device) {
  ...
  if (label.empty() && labels_list.empty() && dynamic_labels_list.empty()) return;
...
void ZoneMap::render_label_text(const char *label, int map_y, int map_x, D3DCOLOR font_color, LabelType label_type,
                                Vec2 offset_pixels) {
  ...
  if ((xf < clip_min_x) || (xf > clip_max_x) || (yf < clip_min_y) || (yf > clip_max_y)) return;
  ...
  if (font_color == D3DCOLOR_XRGB(0, 0, 0)) font_color = D3DCOLOR_XRGB(192, 192, 192);  // Flip to light grey.
...
void ZoneMap::update_mouse() {
  POINT new_mouse_pt = POINT({.x = *Zeal::Game::mouse_client_x, .y = *Zeal::Game::mouse_client_y});
  if (external_enabled) {
    POINT cursor;
  ...
  if (mouse_pt.x < viewport.X || mouse_pt.x >= (viewport.X + viewport.Width) || mouse_pt.y < viewport.Y ||
      mouse_pt.y >= (viewport.Y + viewport.Height)) {
    mouse_drag_enabled = false;
    restore_cursor();
    return;
  }

  bool lmb = external_enabled ? (GetAsyncKeyState(VK_LBUTTON) & 0x8000) : *Zeal::Game::is_left_mouse_down;
...
void ZoneMap::render_handle_cursor(IDirect3DDevice8 &device) {
  ...
  if (xf > clip_min_x && xf < clip_max_x && yf > clip_min_y && yf < clip_max_y) {
    ...
    // Do some logic to overcomb the game.dll's constant decrementing of ShowCursor().
    ...
    if (external_enabled && external_hwnd && GetClientRect(external_hwnd, &win_rect) &&
      ...
      bool in_window = (mouse_pt.x >= win_rect.left && mouse_pt.x < win_rect.right && mouse_pt.y >= win_rect.top &&
                        mouse_pt.y < win_rect.bottom);
      CURSORINFO ci = {sizeof(CURSORINFO)};
      if (in_window && GetCursorInfo(&ci) && !(ci.flags & CURSOR_SHOWING))
        while (ShowCursor(true) < 1)
          ;  // Increment until visible.
    }
  }
}

void ZoneMap::hide_cursor() {
  // In external mode, the game.dll logic is typically setting the thread's ShowCursor() counter
...
void ZoneMap::process_left_mouse_button_down(int16_t mouse_x, int16_t mouse_y) {
  ...
  if ((!map_interactive_enabled && !external_enabled) || !self || zone_id == kInvalidZoneId) return;
...
void ZoneMap::process_mouse_wheel(int16_t mouse_delta, uint16_t flags, int16_t mouse_x, int16_t mouse_y) {
  ...
  if (zone_id == kInvalidZoneId) return;

  POINT corner_pt = {0};
  if (external_enabled) ClientToScreen(external_hwnd, &corner_pt);
...
void ZoneMap::process_on_move(const Zeal::GameUI::CXRect &rect) {
  if (external_enabled || !map_interactive_enabled)  // Shouldn't happen but protect against.
    return;

  if (!is_autocenter_enabled()) manual_on_move_trigger = true;
}
...
void ZoneMap::add_position_marker_vertices(float map_y, float map_x, float heading, float size, D3DCOLOR color,
  ...
  Vec3 symbol[kPositionVertices] = {vertex0, vertex1, vertex3, vertex0, vertex3, vertex2};

  for (int i = 0; i < kPositionVertices; ++i) {
    vertices.push_back(MapVertex{.x = symbol[i].x + map_x, .y = symbol[i].y + map_y, .z = 0.5f, .color = color});
  }
}
...
void ZoneMap::add_group_member_position_vertices(std::vector<MapVertex> &vertices) const {
  ...
  if (map_show_group_mode == ShowGroupMode::kOff || !group_info->is_in_group()) return;
  ...
  const D3DCOLOR kGroupColorLut[GAME_NUM_GROUP_MEMBERS] = {
  ...
  for (int i = 0; i < GAME_NUM_GROUP_MEMBERS; ++i) {
    ...
    if ((strlen(group_info->Names[i]) == 0) || !member || member->Type != Zeal::GameEnums::EntityTypes::Player)
      continue;  // Not a valid group member, out of zone (nullptr), or corpse.

    // Position is y,x,z.
    auto color = (member == Zeal::Game::get_target()) ? get_target_color() : kGroupColorLut[i];
    add_position_marker_vertices(-member->Position.x, -member->Position.y, member->Heading, size, color, vertices);
  }
}

void ZoneMap::add_self_pet_position_vertices(std::vector<MapVertex> &vertices) const {
  if (map_show_group_mode == ShowGroupMode::kOff) return;

  Zeal::GameStructures::Entity *self = Zeal::Game::get_self();
  if (self && self->ActorInfo && self->ActorInfo->PetID) {
    auto pet_entity = Zeal::Game::get_entity_by_id(self->ActorInfo->PetID);
    if (pet_entity) {
...
void ZoneMap::render_group_member_labels(IDirect3DDevice8 &device) {
  ...
  if ((map_show_group_mode == ShowGroupMode::kOff) ||
      (!map_show_all_names_override && (map_show_group_mode == ShowGroupMode::kMarkers)) || !group_info->is_in_group())
    return;
  ...
  for (int i = 0; i < GAME_NUM_GROUP_MEMBERS; ++i) {
    ...
    if (!map_show_all_names_override && map_show_group_mode == ShowGroupMode::kNumbers)
      ...
      for (int j = 0; j < short_name_length; ++j) label[j] = group_info->Names[i][j];
...
void ZoneMap::render_raid_member_labels(IDirect3DDevice8 &device) {
  ...
  if (!map_show_raid || !map_show_all_names_override || !raid_info->is_in_raid()) return;

  render_load_font(device);
  auto entity_manager = ZealService::get_instance()->entity_manager.get();  // Short-term ptr.
  if (!bitmap_font || !entity_manager) return;
  ...
  for (int i = 0; i < kRaidMaxMembers; ++i) {
    ...
    if (strlen(member.Name) == 0) continue;
    auto entity = entity_manager->Get(member.Name);
    if (!entity || entity->Type != Zeal::GameEnums::EntityTypes::Player) continue;  // Could be out of zone or a corpse.
    ...
    for (int j = 0; j < short_name_length; ++j) label[j] = entity->Name[j];
    render_label_text(label, -loc_y, -loc_x, color, LabelType::PositionLabel, offset_pixels);
  }
  bitmap_font->flush_queue_to_screen();
  device.SetVertexShader(kMapVertexFvfCode);  // Restore assumed shader.
...
void ZoneMap::render_non_ally_labels(IDirect3DDevice8 &device,
                                     const std::vector<Zeal::GameStructures::Entity *> &entities) {
  if (!map_show_all_names_override || entities.empty()) return;

  render_load_font(device);
  if (!bitmap_font || entities.empty()) return;
  ...
  for (const auto &entity : entities) {
    ...
    auto color = (entity == target)                          ? get_target_color()
                 : (entity->Type == Zeal::GameEnums::Player) ? player_color
                                                             : npc_color;
...
void ZoneMap::add_raid_marker_vertices(const Vec3 &position, float size, D3DCOLOR color,
                                       std::vector<MapVertex> &vertices) const {
  ...
  for (int i = 0; i < kRaidPositionVertices; ++i) {
    vertices.push_back(MapVertex{.x = symbol[i].x + -position.y,  // Note y,x,z and negation.
...
void ZoneMap::add_non_ally_player_marker_vertices(const Vec3 &position, float size, D3DCOLOR color,
                                                  std::vector<MapVertex> &vertices) const {
...
void ZoneMap::add_npc_marker_vertices(const Vec3 &position, float size, D3DCOLOR color,
                                      std::vector<MapVertex> &vertices) const {
...
void ZoneMap::add_raid_member_position_vertices(std::vector<MapVertex> &vertices) const {
  const Zeal::GameStructures::RaidInfo *raid_info = Zeal::Game::RaidInfo;
  if (!map_show_raid || !raid_info->is_in_raid()) return;

  Zeal::GameStructures::Entity *self = Zeal::Game::get_self();
  auto entity_manager = ZealService::get_instance()->entity_manager.get();  // Short-term ptr.
  if (!entity_manager || !self) return;
  ...
  const DWORD kUngrouped = Zeal::GameStructures::RaidMember::kRaidUngrouped;
  ...
  if (map_show_group_mode != ShowGroupMode::kOff) {
    ...
    for (int i = 0; i < kRaidMaxMembers; ++i) {
      ...
      if (strcmp(member.Name, self->Name) == 0) {
        if (member.GroupNumber != kUngrouped) self_group_number = member.GroupNumber;
        break;
      }
    }
  }
  for (int i = 0; i < kRaidMaxMembers; ++i) {
    ...
    auto color = (entity == Zeal::Game::get_target()) ? get_target_color()
    ...
    if (setting_show_all_player_headings.get())
      add_position_marker_vertices(-entity->Position.x, -entity->Position.y, entity->Heading, size * 0.7f, color,
                                   vertices);
    else
...
std::vector<Zeal::GameStructures::Entity *> ZoneMap::get_non_ally_entities() const {
  std::vector<Zeal::GameStructures::Entity *> entities;
  if (!map_show_all || (!map_show_raid && !Zeal::Game::get_target())) return entities;  // Empty list.

  // First we make a list of the allied entities.
  Zeal::GameStructures::Entity *self = Zeal::Game::get_self();
  auto entity_manager = ZealService::get_instance()->entity_manager.get();  // Short-term ptr.
  if (!entity_manager || !self) return entities;                            // Empty list.

  const auto group_info = Zeal::Game::GroupInfo;
  const auto raid_info = Zeal::Game::RaidInfo;
  std::vector<Zeal::GameStructures::Entity *> allies;
  if (raid_info->is_in_raid()) {
    for (int i = 0; i < kRaidMaxMembers; ++i) {
      const auto &member = raid_info->MemberList[i];
      if (member.Name[0] == 0) continue;
      auto entity = entity_manager->Get(member.Name);
      if (entity && entity->Type == Zeal::GameEnums::EntityTypes::Player) allies.push_back(entity);
    }
  } else if (group_info->is_in_group()) {
    allies.push_back(self);
    for (int i = 0; i < GAME_NUM_GROUP_MEMBERS; ++i) {
      Zeal::GameStructures::Entity *entity = group_info->EntityList[i];
      if (entity) allies.push_back(entity);
    }
  } else
    allies.push_back(self);
...
void ZoneMap::add_non_ally_position_vertices(std::vector<MapVertex> &vertices,
                                             const std::vector<Zeal::GameStructures::Entity *> &entities) const {
  ...
  for (const auto &entity : entities) {
    D3DCOLOR color = (entity == target) ? get_target_color() : (Zeal::Game::GetLevelCon(entity) | 0xff000000);
    if (entity->Position.z < clip_min_z || entity->Position.z > clip_max_z)
      color = color & 0x80ffffff;  // Set alpha to 50% to fade for z.
    ...
    if ((vertices.size() - start) > kVertexLimit) break;  // Note: Dropping markers, but names will still show up.
  }
}
...
void ZoneMap::render_positions(IDirect3DDevice8 &device) {
  ...
  if (!position_vertex_buffer) {
    if (FAILED(device.CreateVertexBuffer(kPositionBufferSize, D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, kMapVertexFvfCode,
                                         D3DPOOL_DEFAULT, &position_vertex_buffer))) {
      position_vertex_buffer = nullptr;
      return;
    }
  }
  ...
  std::vector<Zeal::GameStructures::Entity *> pvp_entities = get_non_ally_entities();
  ...
  bool use_default_color = (map_level_index <= 0) || (position.z >= clip_min_z && position.z <= clip_max_z);
  ...
  add_position_marker_vertices(-position.x, -position.y, self->Heading, size, color, position_vertices);

  int copy_size = position_vertices.size() * sizeof(position_vertices[0]);
  if (copy_size > kPositionBufferSize) return;  // Error. Don't overrun buffer, just hide the markers.
...
// Translate from pixels scale to model (game & map) scale.
...
Vec3 ZoneMap::transform_matrix(const D3DXMATRIX &matrix, const Vec3 &vec) const {
  ...
  return Vec3(matrix(0, 0) * vec[0] + matrix(1, 0) * vec[1] + matrix(2, 0) * vec[2] + matrix(3, 0) * w,
...
Vec3 ZoneMap::transform_model_to_world(const Vec3 &model) const { return transform_matrix(mat_model2world, model); }
...
Vec3 ZoneMap::transform_world_to_model(const Vec3 &world) const { return transform_matrix(mat_world2model, world); }
...
void ZoneMap::render_update_marker_buffer(IDirect3DDevice8 &device) {
  ...
  for (const auto &marker : markers_list) {
    for (int i = 0; i < kNumVertices; ++i) {
      marker_vertices.push_back({.x = marker_symbol[i].x - marker.loc_x,  // Note negation world to map.
  ...
  if (FAILED(device.CreateVertexBuffer(marker_buffer_size, D3DUSAGE_WRITEONLY, kMapVertexFvfCode, D3DPOOL_MANAGED,
...
void ZoneMap::synchronize_external_window() {
  if (!enabled || !external_enabled || !external_hwnd) return;
...
int ZoneMap::get_zlevel_scale() const {
  if (zlevel_height_scale_override > 0) return zlevel_height_scale_override;
  ...
  switch (zone_id) {
    ...
    default:
      ...
      return (Zeal::Game::ZoneInfo->TimeType == 1) ? 2000 : kDefaultZLevelHeightScale;
  }
}

// Returns true  if a map update is needed due to a z level change.
bool ZoneMap::is_zlevel_change() const {
  if (map_level_index != -1) return false;  // Not in auto-mode.
...
void ZoneMap::callback_render() {
  ...
  if (!enabled || !Zeal::Game::is_in_game() || !wnd) return;
  ...
  if (external_enabled && IsIconic(external_hwnd)) return;
  ...
  Zeal::GameStructures::Entity *self = Zeal::Game::get_self();
  IDirect3DDevice8 *device = external_enabled ? external_d3ddev : ZealService::get_instance()->dx->GetDevice();
...
void ZoneMap::add_dynamic_label(const std::string &label_text, int loc_y, int loc_x, unsigned int duration_ms,
                                D3DCOLOR font_color) {
  if (label_text.empty() || zone_id == kInvalidZoneId) return;
...
const ZoneMapData *ZoneMap::get_zone_map(int zone_id) {
  ...
  if (map_data_mode == MapDataMode::kInternal) return internal_map;
  ...
  std::string short_name =
      internal_map ? std::string(internal_map->name) : Zeal::Game::get_zone_name_from_index(zone_id);
  if (short_name.empty()) return internal_map;
...
bool ZoneMap::add_map_data_from_file(const std::string &filename, CustomMapData &map_data) {
  ...
  while (std::getline(map_file, line)) {
    ...
    char buffer[64];
    if (sscanf_s(line.c_str(), "L %f, %f, %f, %f, %f, %f, %u, %u, %u", &x0, &y0, &z0, &x1, &y1, &z1, &red, &green,
                 &blue) == 9) {
      map_data.lines.emplace_back(static_cast<int>(x0 + 0.5f), static_cast<int>(y0 + 0.5f), static_cast<int>(z0 + 0.5f),
...
void ZoneMap::assemble_zone_map(const char *zone_name, CustomMapData &map_data) {
  ...
  std::stable_sort(map_data.lines.begin(), map_data.lines.end(),
                   [](const ZoneMapLine &lhs, const ZoneMapLine &rhs) { return lhs.z0 + lhs.z1 < rhs.z0 + rhs.z1; });
  ...
  if (!map_data.levels.size()) map_data.levels.emplace_back(0, max_z, min_z);

  // Assemble and assign the zone_map_data.
  map_data.zone_map_data =
      std::make_unique<ZoneMapData>(ZoneMapData({.name = map_data.name.c_str(),
                                                 .max_x = max_x,
                                                 .min_x = min_x,
                                                 .max_y = max_y,
                                                 .min_y = min_y,
...
void ZoneMap::set_enabled(bool _enabled, bool update_default) {
  ...
  if (!_enabled) {
    ...
    if (!show_external_window()) {
      Zeal::Game::print_chat("Falling back to internal map overlay");
      external_enabled = false;
    }
  } else if (!external_enabled && map_interactive_enabled) {
    ui_show();
  }
  if (!enabled && _enabled) zone_id = kInvalidZoneId;  // Triggers reload of map.
  enabled = _enabled;
...
void ZoneMap::set_external_enable(bool _external_enabled, bool update_default) {
  if (external_enabled != _external_enabled) {
    ...
    reset_mouse();
    if (was_enabled) set_enabled(true, false);
  }
...
void ZoneMap::set_show_all_names_override(bool flag) { map_show_all_names_override = flag; }

void ZoneMap::set_interactive_enable(bool enable, bool update_default) {
  map_interactive_enabled = enable;
  if (map_interactive_enabled) {
    if (enabled && !external_enabled) ui_show();
  } else {
    ui_hide();
    if (!external_enabled) {
      enable_autocenter();
...
bool ZoneMap::set_grid_pitch(int new_pitch, bool update_default) {
  ...
  if (!map_show_grid) set_show_grid(true, false);  // Turn on the grid if off.
...
static int get_tracking_distance() {
  ...
  const uint8_t zone_type = Zeal::Game::ZoneInfo->TimeType;
...
void ZoneMap::set_marker(int y, int x, const char *label) {
  ...
  const ZoneMapData *zone_map_data = get_zone_map(zone_id);
  if (!zone_map_data || (-x < zone_map_data->min_x || -x > zone_map_data->max_x || -y < zone_map_data->min_y ||
                         -y > zone_map_data->max_y))
    return;
...
bool ZoneMap::set_map_zoom_default_index(int new_index, bool update_default) {
  ...
  set_zoom(static_cast<int>(100 * kDefaultZoomFactors[map_zoom_default_index] +
                            0.5f));  // Also triggers update_ui_options();
...
void ZoneMap::load_ini(IO_ini *ini) {
  if (!ini->exists("Zeal", "MapEnabled")) ini->setValue<bool>("Zeal", "MapEnabled", false);
  if (!ini->exists("Zeal", "MapInteractiveEnabled")) ini->setValue<bool>("Zeal", "MapInteractiveEnabled", false);
  if (!ini->exists("Zeal", "MapExternalEnabled")) ini->setValue<bool>("Zeal", "MapExternalEnabled", false);
  if (!ini->exists("Zeal", "MapDefaultToAutofade")) ini->setValue<bool>("Zeal", "MapDefaultToAutofade", false);
  if (!ini->exists("Zeal", "MapExternalLeftOffset")) ini->setValue<int>("Zeal", "MapExternalLeftOffset", 16);
  if (!ini->exists("Zeal", "MapExternalTopOffset")) ini->setValue<int>("Zeal", "MapExternalTopOffset", 16);
  if (!ini->exists("Zeal", "MapExternalWidth")) ini->setValue<int>("Zeal", "MapExternalWidth", 640);
  if (!ini->exists("Zeal", "MapExternalHeight")) ini->setValue<int>("Zeal", "MapExternalHeight", 640);
  if (!ini->exists("Zeal", "MapZoomDefaultIndex")) ini->setValue<int>("Zeal", "MapZoomDefaultIndex", 0);
  if (!ini->exists("Zeal", "MapShowRaid")) ini->setValue<bool>("Zeal", "MapShowRaid", false);
  if (!ini->exists("Zeal", "MapShowGrid")) ini->setValue<bool>("Zeal", "MapShowGrid", false);
  if (!ini->exists("Zeal", "MapGridPitch")) ini->setValue<int>("Zeal", "MapGridPitch", kDefaultGridPitch);
  if (!ini->exists("Zeal", "MapNameLength")) ini->setValue<int>("Zeal", "MapNameLength", map_name_length);
  ...
  if (!ini->exists("Zeal", "MapPositionSize")) ini->setValue<float>("Zeal", "MapPositionSize", kDefaultPositionSize);
  if (!ini->exists("Zeal", "MapMarkerSize")) ini->setValue<float>("Zeal", "MapMarkerSize", kDefaultMarkerSize);
...
void ZoneMap::save_ini() {
  ...
  ini->setValue<std::string>("Zeal", "MapFontFilename", font_filename.empty() ? std::string("default") : font_filename);
}
...
bool ZoneMap::set_map_rect(float top, float left, float bottom, float right) {
  if (!max_viewport_width || !max_viewport_height) return false;

  if (!wnd || wnd->IsMinimized || external_enabled) return false;
  ...
  map_client_rect = Zeal::GameUI::CXRect(left_edge, top_edge, right_edge, bottom_edge);
  ...
  rect.Right = rect.Right - wnd->Location.Right;
  rect.Bottom = rect.Bottom - wnd->Location.Bottom;
  Zeal::Game::GameInternal::CXWndMoveAndInvalidate(wnd, 0, left_edge - rect.Left, top_edge - rect.Top,
...
void ZoneMap::parse_external(const std::vector<std::string> &args) {
  ...
  Zeal::Game::print_chat("Usage: /map external (toggles on and off)");
}
...
void ZoneMap::parse_rect(const std::vector<std::string> &args) {
  ...
  if ((tlbr.size() != 4) || !set_map_rect(tlbr[0] / 100.f, tlbr[1] / 100.f, tlbr[2] / 100.f, tlbr[3] / 100.f)) {
    Zeal::Game::print_chat("Usage: /map rect <top> <left> <bottom> <right> (percent of screen dimensions)");
    Zeal::Game::print_chat("Example: /map rect 10 20 50 65");
  }
}
...
void ZoneMap::parse_size(const std::vector<std::string> &args) {
  ...
  {
    Zeal::Game::print_chat("Usage: /map size <top> <left> <height> <width> (percent of screen dimensions)");
    Zeal::Game::print_chat("Example: /map size 10 20 40 45");
  }
}
...
void ZoneMap::parse_alignment(const std::vector<std::string> &args) {
  ...
  if ((alignment < AlignmentType::kFirst) || !set_alignment(alignment, false)) {
    Zeal::Game::print_chat("Usage: /map alignment left,center,right");
  }
}
...
void ZoneMap::parse_labels(const std::vector<std::string> &args) {
  ...
  if (args.size() == 6) {
    int y, x, time_ms;
    if (Zeal::String::tryParse(args[3], &y) && Zeal::String::tryParse(args[4], &x) &&
        Zeal::String::tryParse(args[5], &time_ms))
  ...
  if ((labels < LabelsMode::kFirst) || !set_labels_mode(labels, false)) {
    Zeal::Game::print_chat("Usage: /map labels off,summary,all,marker");
  }
}
...
void ZoneMap::parse_show_group(const std::vector<std::string> &args) {
  int length = 0;
  if (args.size() == 2) {
    set_show_group_mode((map_show_group_mode == ShowGroupMode::kOff) ? ShowGroupMode::kMarkers : ShowGroupMode::kOff,
                        false);
    Zeal::Game::print_chat("Showing group members is %s", (map_show_group_mode != ShowGroupMode::kOff) ? "ON" : "OFF");
  } else if (args.size() == 3 && args[2] == "markers") {
    set_show_group_mode(ShowGroupMode::kMarkers, false);
    Zeal::Game::print_chat("Showing group members");
  } else if (args.size() == 3 && args[2] == "numbers") {
    set_show_group_mode(ShowGroupMode::kNumbers, false);
    Zeal::Game::print_chat("Showing group members with numbers");
  } else if (args.size() == 3 && args[2] == "names") {
    set_show_group_mode(ShowGroupMode::kNames, false);
    Zeal::Game::print_chat("Showing group members with names");
  } else if (args.size() == 4 && args[2] == "length" && Zeal::String::tryParse(args[3], &length) &&
             set_name_length(length, false)) {
    Zeal::Game::print_chat("Setting map name label length to: %i", length);
  } else {
    Zeal::Game::print_chat("Usage: /map show_group (toggles on/off), /map show_group [labels_off, numbers, names]");
    Zeal::Game::print_chat("Usage: /map show_group length <value (3 to 20)>");
  }
}
...
bool ZoneMap::search_poi(const std::string &search_term) {
  Zeal::GameStructures::Entity *self = Zeal::Game::get_self();
  if (!self) return false;  // Not handled here.

  const ZoneMapData *zone_map_data = get_zone_map(zone_id);
  if (!zone_map_data || zone_map_data->num_labels < 0) {
    Zeal::Game::print_chat("No map POI data available for this zone");
    return false;
  }
  ...
  for (int i = 0; i <= zone_map_data->num_labels; ++i) {
    ...
    if (label_lower.find(search_lower) != std::string::npos) {
      ...
      const char *flag = "-";
      if (match_count == 1) {
        Zeal::Game::print_chat("Map poi search results for: %s:", search_term.c_str());
      ...
      Zeal::Game::print_chat("%s[%i]: (%i, %i): %s", flag, i, -label.y, -label.x, label.label);
    }
  }
  if (match_count == 0) {
    Zeal::Game::print_chat("%s is not a poi match", search_term.c_str());
    return false;
  }
  return true;
}
...
void ZoneMap::parse_background(const std::vector<std::string> &args) {
  ...
  if ((args.size() == 3 || args.size() == 4) && Zeal::String::tryParse(args[2], &state) &&
    ...
    int alpha = 100;
    if (Zeal::String::tryParse(args[3], &alpha) && set_background_alpha(alpha, false)) return;
  }
  Zeal::Game::print_chat("Usage: /map background <%i to %i> [alpha]", BackgroundType::kFirst, BackgroundType::kLast);
}
...
void ZoneMap::parse_zoom(const std::vector<std::string> &args) {
  ...
  if (args.size() != 3 || !Zeal::String::tryParse(args[2], &zoom_percent) || !set_zoom(zoom_percent))
    Zeal::Game::print_chat("Usage: /map zoom <percent> (<= 100 disables)");
}

int ZoneMap::find_zone_id(const std::string &zone_name) const {
  // First search the client's available zones for a match.
  int index = Zeal::Game::get_index_from_zone_name(zone_name);
  if (index != Zeal::Game::kInvalidZoneId) return index;
  ...
  for (int i = 0; i < 1200; ++i) {
    ...
    if (zone_map_data && zone_name == zone_map_data->name) return i;
  }
...
bool ZoneMap::set_show_zone_id(const std::string &zone_name) {
  ...
  if (new_id == kInvalidZoneId) {
    Zeal::Game::print_chat("Invalid zone name: %s", zone_name.c_str());
    return false;
  }
  ...
  if (self && self->ZoneId != new_id) {
    ...
    Zeal::Game::print_chat("Map showing zone[%i]: %s", show_zone_id, zone_name.c_str());
  }
...
void ZoneMap::parse_show_zone(const std::vector<std::string> &args) {
  ...
  if (args.size() == 3 && set_show_zone_id(args[2])) return;

  Zeal::Game::print_chat("Usage: /map show_zone <short_name> (blank name resets to normal mode)");
}

void ZoneMap::parse_world_data(const std::vector<std::string> &args) {
  if (args.size() == 3 && args[2] == "dump") {
    for (int i = 0; i < Zeal::Game::kNumZoneIds; ++i) {
      std::string short_name = Zeal::Game::get_zone_name_from_index(i);
      std::string long_name = Zeal::Game::get_full_zone_name(i);
      if (!short_name.empty()) {
        Zeal::Game::print_chat("[%i]: %s, %s", i, short_name.c_str(), long_name.c_str());
        int reverse_lookup = Zeal::Game::get_index_from_zone_name(short_name.c_str());
        if (reverse_lookup != i) Zeal::Game::print_chat("[%i]: Reverse mismatch %i", i, reverse_lookup);
      }
    }
    return;
  }
  if (args.size() == 4 && args[2] == "search") {
    Zeal::Game::print_chat("Searching for %s", args[3].c_str());
    for (int i = 0; i < Zeal::Game::kNumZoneIds; ++i) {
      std::string short_name = Zeal::Game::get_zone_name_from_index(i);
      std::string long_name = Zeal::Game::get_full_zone_name(i);
      ...
      if (short_name.find(search) != std::string::npos || long_name.find(search) != std::string::npos)
        Zeal::Game::print_chat("[%i]: %s, %s", i, short_name.c_str(), long_name.c_str());
    }
    return;
  }

  Zeal::Game::print_chat("Usage: /map world dump, /map world search <substring>");
}
...
void ZoneMap::parse_grid(const std::vector<std::string> &args) {
  ...
  } else if (args.size() != 3 || !Zeal::String::tryParse(args[2], &grid_pitch) || !set_grid_pitch(grid_pitch, false))
    Zeal::Game::print_chat("Usage: /map grid [pitch] (blank pitch toggles)");
}
...
void ZoneMap::toggle_level_down() { set_level(map_level_index - 1); }
...
bool ZoneMap::set_level(int level_index) {
  ...
  const ZoneMapData *zone_map_data = get_zone_map(zone_id);
  if (zone_id == kInvalidZoneId || !zone_map_data || zone_map_data->num_levels < 2) return false;
  ...
  if (level_index < minimum_index || level_index >= zone_map_data->num_levels) return false;  // Ineligible index.
...
void ZoneMap::parse_level(const std::vector<std::string> &args) {
  ...
  if (!zone_map_data || zone_map_data->num_levels < 2) {
    Zeal::Game::print_chat("No map level data available for this zone");
    return;
  }

  int value = 0;
  if (args.size() == 2) {
    Zeal::Game::print_chat("Levels for zone: %s", zone_map_data->name);
    for (int i = 0; i < zone_map_data->num_levels; ++i) {
      const ZoneMapLevel &level = zone_map_data->levels[i];
      Zeal::Game::print_chat("[%i]: min_z: %i, max_z: %i, id: %i", i, level.min_z, level.max_z, level.level_id);
    }
    ...
    Zeal::Game::print_chat("Usage: /map level <index> (0: to show all levels, -1: auto z-level)");
    Zeal::Game::print_chat("Usage: /map level alpha <0-100>, /map level autoz <height>");
  }
}
...
void ZoneMap::parse_poi(const std::vector<std::string> &args) {
  ...
  int poi = 0;
  if (args.size() == 2) {
    Zeal::Game::print_chat("Map POI list for zone: %s", zone_map_data->name);
    for (int i = 0; i <= zone_map_data->num_labels; ++i) {
      const ZoneMapLabel &label = (i == 0) ? succor_label : zone_map_data->labels[i - 1];
      Zeal::Game::print_chat("[%i]: (%i, %i): %s", i, -label.y, -label.x, label.label);
    }
  } else if (args.size() == 3 && Zeal::String::tryParse(args[2], &poi, true)) {
    if (poi < 0 || poi > zone_map_data->num_labels) {
      Zeal::Game::print_chat("Invalid selection index");
      return;
    }
    ...
    Zeal::Game::print_chat("Usage: /map poi (displays list of all points of interest)");
    Zeal::Game::print_chat("Usage: /map poi <#> (drops a marker at numeric list [#])");
    Zeal::Game::print_chat("Usage: /map poi <phrase> (search poi labels for phrase)");
    Zeal::Game::print_chat("Example: /map poi 2 (drops a marker at list item [2])");
    Zeal::Game::print_chat("Example: /map poi cazic (searches poi labels for 'cazic')");
  }
}

void print_matrix(const D3DMATRIX &matrix, const char *name) {
  Zeal::Game::print_chat(name);
  for (int i = 0; i < 4; ++i) {
    Zeal::Game::print_chat("[%i]: %f, %f, %f, %f", i, matrix.m[i][0], matrix.m[i][1], matrix.m[i][2], matrix.m[i][3]);
  }
}

void ZoneMap::dump() {
  Zeal::Game::print_chat("-----Map-----");
  Zeal::Game::print_chat("enabled: %i, inter_enable: %i, ext_enable: %i, reenable: %i, zone_id: %i", enabled,
                         map_interactive_enabled, external_enabled, reenable_on_zone, zone_id);
  Zeal::Game::print_chat("background: %i (%.2f), grid: %i, align: %i, labels:%i, show_zone: %i", map_background_state,
                         map_background_alpha, map_show_grid ? map_grid_pitch : 0, map_alignment_state, map_labels_mode,
                         show_zone_id);
  Zeal::Game::print_chat("num_markers: %i, num_labels: %i, name_length: %i, ring: %i", markers_list.size(),
                         labels_list.size(), map_name_length, map_ring_radius);
  Zeal::Game::print_chat("view: t: %i, l: %i, h: %i, w: %i, Max H: %i, W: %i", viewport.Y, viewport.X, viewport.Height,
                         viewport.Width, max_viewport_height, max_viewport_width);
  if (wnd) {
    const Zeal::GameUI::CXRect &loc = wnd->IsMinimized ? wnd->LocationPlaceholder : wnd->Location;
    Zeal::Game::print_chat("wnd: t: %i, l: %i, b: %i, r: %i", loc.Top, loc.Left, loc.Bottom, loc.Right);
  }
  Zeal::Game::print_chat("client: t: %i, l: %i, b: %i, r: %i", map_client_rect.Top, map_client_rect.Left,
                         map_client_rect.Bottom, map_client_rect.Right);
  Zeal::Game::print_chat("clip: t: %f, l: %f, b: %f, r: %f", clip_min_y, clip_min_x, clip_max_y, clip_max_x);
  Zeal::Game::print_chat("level: index: %i, z_max: %i, z_min: %i, faded_alpha: %.2f, z_scale: %i", map_level_index,
                         clip_max_z, clip_min_z, map_faded_zlevel_alpha,
                         zlevel_height_scale_override > 0 ? zlevel_height_scale_override : zlevel_height_scale);
  Zeal::Game::print_chat("position_size: %f, marker_size: %f, show_group: %i, show_raid: %i", position_size,
                         marker_size, static_cast<int>(map_show_group_mode), map_show_raid);
  Zeal::Game::print_chat("scale_factor: %f, offset_y: %f, offset_x: %f, zoom: %f", mat_model2world(0, 0),
                         mat_model2world(3, 1), mat_model2world(3, 0), zoom_factor);
  Zeal::Game::print_chat("dyn_labels_size: %i, data_mode: %i, data_cache: %i", dynamic_labels_list.size(),
                         map_data_mode, map_data_cache.size());
  Zeal::Game::print_chat("line_count: %i, grid: %i, line: %i, position: %i, marker: %i, font: %i (%s)", line_count,
                         grid_line_count, line_vertex_buffer != nullptr, position_vertex_buffer != nullptr,
                         marker_vertex_buffer != nullptr, bitmap_font != nullptr, font_filename.c_str());
  Zeal::Game::print_chat("interactive: (%i, %i) = (%f, %f), mouse: (%i, %i)", manual_screen_pt.x, manual_screen_pt.y,
                         manual_position.x, manual_position.y, mouse_pt.x, mouse_pt.y);
  Zeal::Game::print_chat("external: enabled: %i, left: %i, top: %i, width: %i, height: %i", external_enabled,
                         external_monitor_left_offset, external_monitor_top_offset, external_monitor_width,
                         external_monitor_height);
  Zeal::Game::print_chat("hinstance: 0x%08x, hwnd: 0x%08x, d3d: 0x%08x, d3ddev: 0x%08x",
  ...
  if (external_hwnd) {
    ...
    Zeal::Game::print_chat("ext win rect top: %i, left: %i, bottom: %i, right %i", win_rect.top, win_rect.left,
                           win_rect.bottom, win_rect.right);

    RECT client_rect = {0};
    GetClientRect(external_hwnd, &client_rect);
    Zeal::Game::print_chat("ext client rect top: %i, left: %i, bottom: %i, right %i", client_rect.top, client_rect.left,
                           client_rect.bottom, client_rect.right);
  }
...
bool ZoneMap::parse_command(const std::vector<std::string> &args) {
  if (args.size() == 1) {
    ...
    Zeal::Game::print_chat("Saving current map settings");
    save_ini();
  } else if (args[1] == "grid") {
    parse_grid(args);
  } else if (args[1] == "ring") {
...
void ZoneMap::callback_zone() {
  if (!wnd) return;  // Block calls until init_ui has been called.

  set_window_title();    // Reset the title to default until loaded.
  map_show_all = false;  // Disable marauders mode on a zone.
  reset_zone_state();
  if (enabled || reenable_on_zone) set_enabled(true, false);  // Calls ui_show() and syncs enable state.
...
void ZoneMap::set_window_title(const char *title) {
  ...
  if (external_hwnd) SetWindowTextA(external_hwnd, title);
}
...
static int __fastcall HandleWheelMove(Zeal::GameUI::SidlWnd *wnd, int unusedEDX, int32_t mouse_x, int32_t mouse_y,
                                      int32_t scroll_delta, uint32_t unused4) {
  ZealService *zeal = ZealService::get_instance();
  if (zeal && zeal->zone_map) zeal->zone_map->process_mouse_wheel(scroll_delta, 0x00, mouse_x, mouse_y);
  return 0;
}

// Use the mouse left button down event since it has undergone proper zlayer window filtering.
static int __fastcall HandleLButtonDown(Zeal::GameUI::SidlWnd *wnd, int unusedEDX, int32_t mouse_x, int32_t mouse_y,
                                        uint32_t unused3) {
  ZealService *zeal = ZealService::get_instance();
  if (zeal && zeal->zone_map) zeal->zone_map->process_left_mouse_button_down(mouse_x, mouse_y);
  return 0;
}

// Use the mouse right button down event since it has undergone proper zlayer window filtering.
static int __fastcall HandleRButtonDown(Zeal::GameUI::SidlWnd *wnd, int unusedEDX, int32_t mouse_x, int32_t mouse_y,
                                        uint32_t unused3) {
  // Need to manually do a simple hit test for a right click in the title bar region.
  Zeal::GameUI::CXRect rect;
  Zeal::Game::GameInternal::CXWndGetClientRect(wnd, 0, &rect);
  if (mouse_y < rect.Top)
    return Zeal::Game::GameInternal::CSidlScreenWndHandleRButtonDown(wnd, unusedEDX, mouse_x, mouse_y, unused3);

  ZealService *zeal = ZealService::get_instance();
  if (zeal && zeal->zone_map) zeal->zone_map->process_right_mouse_button_down(mouse_x, mouse_y);
  return 0;
}

// Note: This callback does appear to be functional in the client code.
static int __fastcall HandleWheelButtonDown(Zeal::GameUI::SidlWnd *wnd, int unusedEDX, int32_t mouse_x, int32_t mouse_y,
                                            uint32_t unused3) {
  ZealService *zeal = ZealService::get_instance();
  if (zeal && zeal->zone_map) zeal->zone_map->process_wheel_mouse_button_down(mouse_x, mouse_y);
  return 0;
}

// Just suppress other mouse events since update_mouse() polling is more reliable.
static int __fastcall DropMouseUpdate(Zeal::GameUI::SidlWnd *wnd, int unusedEDX, int32_t mouse_x, int32_t mouse_y,
                                      uint32_t unused3) {
  return 0;
}
...
static void __fastcall Deactivate(Zeal::GameUI::SidlWnd *wnd, int unusedEDX) {
  wnd->show(0, false);
  return;
}

// Could be used to detect the map getting closed by the title bar button.
static int __fastcall OnShow(Zeal::GameUI::SidlWnd *wnd, int unusedEDX) {
  return Zeal::Game::GameInternal::CSidlScreenWndOnShow(wnd, unusedEDX);
}

static int __fastcall OnMove(Zeal::GameUI::SidlWnd *wnd, int unusedEDX, Zeal::GameUI::CXRect rect) {
  // Zeal::Game::print_chat("OnMove: l: %i t: %i r: %i b: %i", rect.Left, rect.Top, rect.Right, rect.Bottom);
  ZealService *zeal = ZealService::get_instance();
  if (zeal && zeal->zone_map) zeal->zone_map->process_on_move(rect);
  return 0;  // The default CXWnd::OnMove() just returns 0.
}

// Constrain the size to reasonable minimums.  Also verified during rendering.
static int __fastcall OnResize(Zeal::GameUI::SidlWnd *wnd, int unusedEDX, int width, int height) {
  // Zeal::Game::print_chat("OnResize: %i x %i", width, height);
  width = max(width, kWinMinSize);
  height = max(height, kWinMinSize);
  ZealService *zeal = ZealService::get_instance();
  if (zeal && zeal->zone_map) zeal->zone_map->process_on_resize(width, height);
  return Zeal::Game::GameInternal::CXWndOnResize(wnd, unusedEDX, width, height);
}

// Hooked to characterize and to allow debug printing when desired.
static void __fastcall OnMinimizeBox(Zeal::GameUI::SidlWnd *wnd, int unusedEDX) {
  Zeal::Game::GameInternal::CXWndOnMinimizeBox(wnd, unusedEDX);
}

// Not currently getting called by the framework.  Base class does nothing anyways.
static int __fastcall OnActivate(Zeal::GameUI::SidlWnd *wnd, int unusedEDX) {
  return Zeal::Game::GameInternal::CXWndOnActivate(wnd, unusedEDX);
}

// Hooked to characterize.
static int __fastcall AboutToDeleteWnd(Zeal::GameUI::SidlWnd *wnd, int unusedEDX) {
  return Zeal::Game::GameInternal::CXWndAboutToDeleteWnd(wnd, unusedEDX);
}

// Hooked to characterize and to allow debug printing when desired.
static Zeal::GameUI::CXRect *__fastcall GetMinimizedRect(Zeal::GameUI::SidlWnd *wnd, int unusedEDX,
                                                         Zeal::GameUI::CXRect *rect) {
  return Zeal::Game::GameInternal::CXWndGetMinimizedRect(wnd, unusedEDX, rect);
}

// Hooked to characterize and to allow debug printing when desired.
static void __fastcall StoreIniInfo(Zeal::GameUI::SidlWnd *wnd, int unusedEDX) {
  Zeal::Game::GameInternal::CSidlScreenWndStoreIniInfo(wnd, unusedEDX);
}

// This callback is required since the Map window isn't tied into the game window list for creation. The
...
void ZoneMap::callback_init_ui() {
  ...
  if ((wnd->Location.Right - wnd->Location.Left < kWinMinSize) ||
    ...
    Zeal::Game::GameInternal::CXWndMoveAndInvalidate(wnd, 0, 0, 0, kWinMinSize, kWinMinSize);
  }

  // Re-initialize to defaults on every reload (after character select). Also lets the enable arm.
  if (ZealService::get_instance() && ZealService::get_instance()->ini && ZealService::get_instance()->ini.get())
    load_ini(ZealService::get_instance()->ini.get());
  else
    Zeal::Game::print_chat("Error: Failed to load map ini options");
...
bool ZoneMap::ui_show() {
  if (!wnd) return false;
  if (!wnd->IsVisible) wnd->show(1, false);
  return wnd->IsVisible;
}

// This callback is required since the Map window isn't tied into the game window list for destruction.
...
// This callback is required since the Map window isn't tied into the game window list for deactivation.
...
ZoneMap::ZoneMap(ZealService *zeal) {
  ...
  zeal->callbacks->AddGeneric([this]() { callback_dx_reset(); }, callback_type::DXCleanDevice);
...
static uint32_t virtual_key_to_dik(uint32_t vk_key) {
  static constexpr UINT kVtoSExceptions[][2] = {{VK_UP, DIK_UP},
                                                {VK_DOWN, DIK_DOWN},
                                                {VK_LEFT, DIK_LEFT},
                                                {VK_RIGHT, DIK_RIGHT},
  ...
  for (int i = 0; i < num_entries; i++) {
    if (kVtoSExceptions[i][0] == vk_key) return kVtoSExceptions[i][1];
  }
...
void process_key(uint32_t vk_key, bool key_down) {
  ...
  // The Game keeps track of all this internally, just poll similarly for external.
  if (GetKeyState(VK_SHIFT) & 0x8000) key_code = key_code | 0x10000000;
  if (GetKeyState(VK_CONTROL) & 0x8000) key_code = key_code | 0x20000000;
  if (GetKeyState(VK_MENU) & 0x8000) key_code = key_code | 0x40000000;

  // Note: Not supporting the proper game call for DIK_ESCAPE functionality (not a command).
  ...
  const uint32_t *key_bind_pri = reinterpret_cast<const uint32_t *>(Zeal::Game::ptr_PrimaryKeyMap);
  const uint32_t *key_bind_alt = reinterpret_cast<const uint32_t *>(Zeal::Game::ptr_AlternateKeyMap);
  for (int i = first_index; i <= lut_size; ++i) {
    if (i == kCommandSlashIndex || i == kCommandTellIndex || i == kCommandReplyIndex || i == kCommandChatHistoryUp ||
        i == kCommandChatHistoryDown)
      continue;  // Skip searching for these commands.
    if (key_bind_pri[i] == key_code || key_bind_alt[i] == key_code) Zeal::Game::execute_cmd(i, key_down, 0);
  }
}
...
LRESULT CALLBACK ZoneMap::external_window_proc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  switch (uMsg) {
    ...
    case WM_MBUTTONDOWN:
      ...
      return 0;
    case WM_MOUSEWHEEL:
      process_mouse_wheel(static_cast<int16_t>(wParam >> 16), wParam & 0xffff, static_cast<int16_t>(lParam & 0xffff),
...
void ZoneMap::hide_external_window() {
  if (external_hwnd) ShowWindow(external_hwnd, SW_HIDE);
}

bool ZoneMap::show_external_window() {
  if (!create_external_window()) {
    Zeal::Game::print_chat("Error creating external map window");
    return false;
  }
...
bool ZoneMap::create_external_window() {
  ...
  if (!GetClassInfoEx(external_hinstance, kExternalWindowClassName, &wcex)) {
    ...
    if (RegisterClassEx(&wcex) == 0) {  // Returns 0 if fails.
      Zeal::Game::print_chat("Error registering external map window class: %i", GetLastError());
      return false;
    }
  }

  // Stored window size is the client area (viewport).
  RECT win_rect = {.left = external_monitor_left_offset,
                   .top = external_monitor_top_offset,
                   .right = external_monitor_left_offset + external_monitor_width,
                   .bottom = external_monitor_top_offset + external_monitor_height};
  AdjustWindowRectEx(&win_rect, WS_OVERLAPPEDWINDOW, false, NULL);
  external_hwnd = CreateWindowEx(NULL, kExternalWindowClassName, kExernalWindowTitle, WS_OVERLAPPEDWINDOW,
                                 win_rect.left, win_rect.top, win_rect.right - win_rect.left,
                                 win_rect.bottom - win_rect.top, NULL, NULL, external_hinstance,
...
bool ZoneMap::initialize_d3d_external_window() {
  ...
  auto result = external_d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, external_hwnd,
                                           D3DCREATE_HARDWARE_VERTEXPROCESSING, &d3dpp, &external_d3ddev);

  if (FAILED(result)) {
    Zeal::Game::print_chat("Error creating external map device: %d (%d x %d)", result, external_width, external_height);
    release_d3d_external_window();
    reset_zone_state();
    return false;
  }
