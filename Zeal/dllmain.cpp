...
static void zeal_lifetime_thread(std::unique_ptr<ZealService> zeal) {
  ...
  if (!exitFlag.load(std::memory_order_acquire)) {
    ...
    zeal.reset();  // Deletes the ZealService (calling deconstructor). Note: cross-thread call with game's thread.
    Sleep(1000);
  ...
  switch (ul_reason_for_call) {
    case DLL_PROCESS_ATTACH: {
      if (this_module || check_if_already_loaded())
        ...
        auto zeal = std::make_unique<ZealService>();  // Construct before game thread is started.
