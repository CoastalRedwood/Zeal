...
DWORD Tick::GetTimeUntilTick() {
  if (LastKnownServerTick != 0)  // Don't calculate until tick is known
  {
    ...
    DWORD millis_until_tick =
        tick_point >= now_point ? tick_point - now_point : kAverageTickDuration - (now_point - tick_point);
    return millis_until_tick;
  }
  return 0;
}

DWORD Tick::GetTickGauge(Zeal::GameUI::CXSTR *str) {
  DWORD value = GetTimeUntilTick();

  // The seconds remaining (1-6)
  DWORD seconds_display = (value / 1000) + 1;
  if (seconds_display > 6) seconds_display = 6;
  Zeal::Game::CXStr_PrintString(str, "%u", seconds_display);

  // Scales the gauge from 0-1000
  value = value * kGaugeScale / kAverageTickDuration;

  // Toggle can reverse the gauge display (drain / fill)
  if (ReverseDirection.get()) value = kGaugeScale - value;
...
Tick::Tick(ZealService *zeal) {
  ...
  zeal->commands_hook->Add("/tickreverse", {}, "Swaps the direction of the tick progress.",
                           [this](std::vector<std::string> &args) {
                             ...
                             Zeal::Game::print_chat("Server Tick direction swapped.");
                             return true;
                           });
}
