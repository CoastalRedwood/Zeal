...
#include "vectors.h"
#define GAME_NUM_COMMANDS 277  // 0-276
#define GAME_NUM_BUFFS 15
#define GAME_NUM_SPELLS 4000
#define GAME_NUM_SPELL_GEMS 8
#define GAME_NUM_GROUP_MEMBERS 5
#define GAME_NUM_INVENTORY_SLOTS 21
#define GAME_NUM_INVENTORY_PACK_SLOTS 8
#define GAME_NUM_INVENTORY_BANK_SLOTS 8
#define GAME_NUM_SKILLS 74
#define GAME_NUM_SPELL_BOOK_SPELLS 512
#define GAME_NUM_SPAWNS 8192
#define GAME_NUM_GUILDS 512
#define GAME_NUM_LOOT_WINDOW_ITEMS 30
...
#define USERCOLOR_SPELL_FAILURE \
  0xFF + 34  //  34 - Spell Failures (resists, fizzles, missing component, bad target, etc.)
...
enum ItemTypes  // From server common/item_data.h
{
...
enum SkillType  // From server common/skills.h
{
...
typedef struct _GAMEITEMCOMMONINFO {
  ...
  /* 0x00FB */ BYTE
      IsStackableEx;  // Effect Type: 0=Combat 1=ClickyAny 2=Worn 3=Expendable 4=ClickyMustEquip 5=ClickyCanEquip
  /* 0x00FC */ BYTE Range;
...
} GAMEITEMCOMMONINFO, *PGAMEITEMCOMMONINFO;

typedef struct _GAMEITEMCONTAINERINFO  // Game_Container class.
{
  /* 0x00E4 */ struct _GAMEITEMINFO *Item[GAME_NUM_CONTAINER_SLOTS];
...
} GAMEITEMCONTAINERINFO, *PGAMEITEMCONTAINERINFO;

typedef struct _GAMEITEMBOOKINFO {
  /* 0x00E4 */ BYTE Unknown0228[3];
  /* 0x00E7 */ CHAR File[15];
  /* ...... */
} GAMEITEMBOOKINFO, *PGAMEITEMBOOKINFO;
...
} GAMEITEMINFOBASE, *PGAMEITEMINFOBASE;

typedef struct _GAMEITEMINFO : public GAMEITEMINFOBASE {
  union {
    /* 0x00E4 */ GAMEITEMCOMMONINFO Common;        // GAMEITEMINFOBASE.Type == 0.
    /* 0x00E4 */ GAMEITEMCONTAINERINFO Container;  // GAMEITEMINFOBASE.Type == 1.
    /* 0x00E4 */ GAMEITEMBOOKINFO Book;            // GAMEITEMINFOBASE.Type == 2.
  };

  /* ...... */
} GAMEITEMINFO, *PGAMEITEMINFO;
...
struct GAMEDAGINFO {
  ...
  /* 0x0008 */ struct _GAMESTRINGSPRITE *StringSprite;
  /* 0x000C */ struct _GAMEDAGINFO *Parent;
  /* 0x0010 */ struct _GAMEMODELINFO *ModelInfo;
  /* 0x0014 */ struct _GAMETRACKINFO *Track1;
  /* 0x0018 */ struct _GAMETRACKINFO *Track2;
  ...
  /* 0x00EC */ struct _GAMEDAGCHILDREN *Children;
  /* ...... */
};

struct _GAMEMODELINFO {
  /* 0x0000 */ BYTE Unknown0000[64];
  /* 0x0040 */ DWORD NumSkinsAttachedToHierarchicalSprite;
  /* ...... */
};

typedef struct _GAMEARGBCOLOR {
  union {
    struct {
      BYTE B;
      BYTE G;
  ...
  _GAMEARGBCOLOR(BYTE _R, BYTE _G, BYTE _B, BYTE _A) : A(_A), R(_R), G(_G), B(_B){};
  _GAMEARGBCOLOR(DWORD _ARGB) : ARGB(_ARGB){};
  _GAMEARGBCOLOR() : A{}, R{}, G{}, B{} {};

} GAMEARGBCOLOR;

typedef struct _GAMESTRINGSPRITE {
  ...
  /* 0x0048 */ GAMEARGBCOLOR Color;  // s3dSetStringSpriteTint
} GAMESTRINGSPRITE, *PGAMESTRINGSPRITE;
...
struct ActorInfo {
  ...
  /* 0x0004 */ struct _GAMELIGHTINFO *LightInfo;  // PointLight
  ...
  /* 0x0050 */ PVOID Unknown0050;  // GameMobileEmitter*
  ...
  /* 0x00D8 */ DWORD RecastTimeout[GAME_NUM_SPELL_GEMS];  // Set in the OP_MemorizeSpell handler.
  ...
  /* 0x0274 */ BYTE IsInvited;  // Set to True in GamePlayer::SetInvited()
  ...
  /* 0x0284 */ struct _GAMEMODELINFO *ModelInfo;
  /* 0x0288 */ GAMEDAGINFO *DagHeadPoint;
  /* 0x028C */ GAMEDAGINFO *DagHead;
  /* 0x0290 */ GAMEDAGINFO *DagUnknown;
  ...
  /* 0x0338 */ BYTE AAAbilities[0xe3];  // Current AA value (used by Game_Character::GetAbility().
...
typedef struct _GAMEINVENTORY {
  /* 00 */ struct _GAMEITEMINFO *EarLeft;
  /* 01 */ struct _GAMEITEMINFO *Head;
  /* 02 */ struct _GAMEITEMINFO *Face;
  /* 03 */ struct _GAMEITEMINFO *EarRight;
  /* 04 */ struct _GAMEITEMINFO *Neck;
  /* 05 */ struct _GAMEITEMINFO *Shoulders;
  /* 06 */ struct _GAMEITEMINFO *Arms;
  /* 07 */ struct _GAMEITEMINFO *Back;
  /* 08 */ struct _GAMEITEMINFO *WristLeft;
  /* 09 */ struct _GAMEITEMINFO *WristRight;
  /* 10 */ struct _GAMEITEMINFO *Ranged;
  /* 11 */ struct _GAMEITEMINFO *Hands;
  /* 12 */ struct _GAMEITEMINFO *Primary;
  /* 13 */ struct _GAMEITEMINFO *Secondary;
  /* 14 */ struct _GAMEITEMINFO *RingLeft;
  /* 15 */ struct _GAMEITEMINFO *RingRight;
  /* 16 */ struct _GAMEITEMINFO *Chest;
  /* 17 */ struct _GAMEITEMINFO *Legs;
  /* 18 */ struct _GAMEITEMINFO *Feet;
  /* 19 */ struct _GAMEITEMINFO *Waist;
  /* 20 */ struct _GAMEITEMINFO *Ammo;
} GAMEINVENTORY, *PGAMEINVENTORY;

struct _GAMEBUFFINFO {
  ...
  /* 0x0003 */ BYTE
      Activated;  // Copied from spell data to buff struct.  Only a few spells have this set to 1, the rest are 0
  /* 0x0004 */ WORD SpellId;
...
struct GAMEZONEINFO {  // OP_NewZone 0x4058 performs a memcpy of the first 0x23c bytes of NewZone_struct packet data to
                       // 0x00798784.
  ...
  /* 0x01A8 */ BYTE TimeType;  // 0 = indoor dungeon, 1 = outdoor, 2 = outdoor city, 3 = indoor city, 4 = nosky outdoor
                               // city, 5 = outdoor dungeon
...
struct GAMECHARINFO  // common/patches/mac_structs.h::PlayerProfile_Struct
{
  ...
  /* 0x0088 */ WORD Gender;        // Game_GENDER_x
  /* 0x008A */ WORD Race;          // Game_RACE_x
  /* 0x008C */ BYTE Class;         // Game_CLASS_x
  ...
  /* 0x009A */ WORD Mana;
  /* 0x009C */ WORD BaseHP;
  /* 0x009E */ WORD StunnedState;  // Game_STUNNED_STATE_x
  /* 0x00A0 */ WORD BaseSTR;
  /* 0x00A2 */ WORD BaseSTA;
  /* 0x00A4 */ WORD BaseCHA;
  /* 0x00A6 */ WORD BaseDEX;
  ...
  /* 0x0264 */ _GAMEBUFFINFO Buff[GAME_NUM_BUFFS];
  /* 0x02FA */ BYTE Unknown02FA[1080];
  /* 0x0732 */ WORD SpellBook[GAME_NUM_SPELL_BOOK_SPELLS];
  /* 0x0B32 */ SHORT MemorizedSpell[GAME_NUM_SPELL_GEMS];  // spell gem spell ids
  /* 0x0B42 */ BYTE Unknown0B42[14];
  /* 0x0B50 */ WORD Unknown0B50;
  /* 0x0B52 */ WORD Unknown0B52;
  /* 0x0B54 */ Vec3 ZoneEnter;
  /* 0x0B60 */ FLOAT Unknown0060;
  /* 0x0B64 */ BYTE StandingState;  // Game_STANDING_STATE_x
  ...
  /* 0x0BA8 */ WORD Skills[GAME_NUM_SKILLS];
  ...
  /* 0x0D54 */ DWORD Hunger;
  /* 0x0D58 */ DWORD Thirst;
  /* 0x0D5C */ _GAMEBUFFINFO BuffsExtIndexMinus15[2];  // The client uses this field to read from 'BuffsExt[n-15]' by
                                                       // passing BuffsExtIndexMinus15[n] to this array, which is the
                                                       // same struct offset. Usage not recommended.
  /* 0x0D70 */ DWORD ZoneId;
  /* 0x0D74 */ Entity *SpawnInfo;
  /* 0x0D78 */ _GAMEITEMINFO *CursorItem;
  union {
    /* 0x0D7C */ struct _GAMEINVENTORY Inventory;
    /* 0x0D7C */ struct _GAMEITEMINFO *InventoryItem[GAME_NUM_INVENTORY_SLOTS];
  };
  /* 0x0DD0 */ struct _GAMEITEMINFO *InventoryPackItem[GAME_NUM_INVENTORY_PACK_SLOTS];
  /* 0x0DF0 */ BYTE Unknown0DF0[2];
  /* 0x0DF2 */ _GAMEBUFFINFO BuffsExt[15];  // non-PCs have access to 15 more buff slots. We can maybe use this space
                                            // for the short-buff window in the future
  ...
  //       for the presence of the patch before accessing beyond GAME_NUM_INVENTORY_BANK_SLOTS.
  //       Currently this is the get_num_total_bank_slots() which returns 60 if patched.
  // Original structure:
  // 0x20B4 */ struct _GAMEITEMINFO* InventoryBankItem[GAME_NUM_INVENTORY_BANK_SLOTS];
  // 0x20D4 */ DWORD UnusedSpace[12]; (Index '4' is set to 1 but unused, patched out).
  // };  /* 0x2104 (total size) */
  // Patched layout:
  /* 0x20B4 */ struct _GAMEITEMINFO *InventoryBankItem[30];
  /* 0x212C */ struct _GAMEITEMINFO *SharedBankItem[30];
}; /* 0x21A4 (total size) */
...
struct Entity {
  ...
  /* 0x00A8 */ BYTE Type;    // SPAWN_TYPE_x
  /* 0x00A9 */ BYTE Class;   // CLASS_x
  ...
  /* 0x00C4 */ WORD EquipmentSecondaryItemType;  // EQUIPMENT_ITEM_TYPE_x ; Secondary
  ...
  /* 0x010A */ WORD LevitationState;                   // LEVITATION_STATE_x
  /* 0x010C */ DWORD TargetType;                       // SPAWN_TARGET_TYPE_x
  /* 0x0110 */ DWORD Unknown0110;
  /* 0x0114 */ DWORD AnonymousState;  // ANONYMOUS_STATE_x, /anonymous and /roleplay
...
struct GroupInfo {
  ...
  /* 0x0000 */ BYTE IsValidList[GAME_NUM_GROUP_MEMBERS];  // 1 = Valid group member, 0 = empty.
  /* 0x0005 */ CHAR Names[GAME_NUM_GROUP_MEMBERS][64];
  /* 0x0145 */ CHAR Unknown0143[3];  // Alignment padding probably.
  /* 0x0148 */ struct Entity *EntityList[GAME_NUM_GROUP_MEMBERS];
...
struct SPELLMGR {
  SPELL *Spells[GAME_NUM_SPELLS];
};

struct GameAltAbility {          // operator new of 0x38 bytes.
