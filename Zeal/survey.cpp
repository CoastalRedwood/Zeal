...
std::string extract_quoted_string(const std::string &input) {
  ...
  if (start == std::string::npos) return "";  // No opening quote found.
  start++;
  size_t end = input.find('\'', start);
  if (end == std::string::npos) return "";  // No closing quote.
...
void Survey::check_message_for_survey_trigger(const char *message) {
  if (!setting_enable.get() || !message || !strstr(message, kZealSurveyHeader)) return;

  std::string message_str = message;
  if (!message_str.starts_with(kZealSurveyHeader)) return;

  auto split = Zeal::String::split_text(message_str, kDelimiter);
  if (split.size() != 3) return;

  if (split[0] != kZealSurveyHeader) return;
...
void Survey::handle_start_of_survey(const std::string &channel, const std::string &question) {
  ...
  if (!survey_channel.starts_with(kZealSurveyPrefix) || survey_question.size() == 0) return;

  // Join the channel to send and receive responses.
  if (Zeal::Game::get_channel_number(survey_channel.c_str()) < 0)
    Zeal::Game::do_join(Zeal::Game::get_self(), survey_channel.c_str());

  if (!ZealService::get_instance()->ui || !ZealService::get_instance()->ui->inputDialog) return;
  ...
  ZealService::get_instance()->ui->inputDialog->show(
      "Zeal Survey", survey_question, "Yes", "No", [this](std::string response) { send_response("Yes"); },
...
bool Survey::check_for_response_message(const char *message, int color_index) {
  ...
  if (channel < 0 || ((USERCOLOR_CHAT_1 + channel) != color_index && (USERCOLOR_ECHO_CHAT_1 + channel) != color_index))
    return false;
  ...
  if (name == "You" && Zeal::Game::get_self()) name = Zeal::Game::get_self()->Name;
  ...
  if (!name.empty() && split.size() == 2 && split[0] == kZealResponseHeader) {
    ...
    for (auto &pair : survey_responses) {
      ...
      if (it != pair.second.end()) pair.second.erase(it);  // Future: Maybe log the change in vote.
    }
    auto it = survey_responses.find(split[1]);
    if (it == survey_responses.end()) survey_responses.insert({split[1], std::vector<std::string>()});
...
void Survey::send_survey(const std::string &channel, const std::string &question) {
  ...
  if (!channel.starts_with(kZealSurveyPrefix)) {
    Zeal::Game::print_chat("Response channel name must start with survey (like 'survey123')");
    return;
  }

  if (question.empty()) {
    Zeal::Game::print_chat("Survey question must not be empty");
    return;
  }

  if (!Zeal::Game::RaidInfo->is_in_raid()) {
    Zeal::Game::print_chat("Must be in a raid to start a survey poll");
    return;
  }

  if (!setting_enable.get()) {
    Zeal::Game::print_chat("Enabling /survey to support sending one");
    setting_enable.set(true);
  }

  std::string message = std::format("{0}{1}{2}{3}{4}", kZealSurveyHeader, kDelimiter, channel, kDelimiter, question);
...
void Survey::send_response(const std::string &response) {
  int channel = Zeal::Game::get_channel_number(survey_channel.c_str());
  if (channel < 0) {
    Zeal::Game::print_chat("Unable to send response to unjoined chat channel %s", survey_channel.c_str());
    return;
  }

  std::string message = std::format("{0}{1}{2}", kZealResponseHeader, kDelimiter, response);
  Zeal::Game::send_to_channel(channel, message.c_str());
  Zeal::Game::print_chat("You responded '%s' to %s", response.c_str(), survey_channel.c_str());
}
...
void Survey::print_results(bool send_to_raidsay) const {
  if (survey_question.empty() || survey_channel.empty()) {
    Zeal::Game::print_chat("No active survey");
    return;
  }

  Zeal::Game::print_chat("Results for question: %s", survey_question.c_str());
  if (survey_responses.empty()) {
    Zeal::Game::print_chat("Zero responses");
    return;
  }
  ...
  for (int i = 0; i < Zeal::GameStructures::RaidInfo::kRaidMaxMembers; ++i) {
    ...
    bool found = false;
    for (const auto &pair : survey_responses) {
      if (std::find(pair.second.begin(), pair.second.end(), name) != pair.second.end()) found = true;
    }
    if (!found) no_responses.push_back(name);
  }
  int num_names = min(25, no_responses.size());  // Limit # of names for now (buffer size < 2048).
  std::string names;
  for (int i = 0; i < num_names; ++i) {
    names += no_responses[i];
    if (i < no_responses.size() - 1) names += ",";
  }
...
bool Survey::parse_command(const std::vector<std::string> &args) {
  if (args.size() > 2 && args[1] == "new") {
    ...
    setting_enable.set(true);
    Zeal::Game::print_chat("/survey is enabled");
  } else if (args.size() == 2 && args[1] == "off") {
    setting_enable.set(false);
    survey_responses.clear();
    survey_question = "";
    Zeal::Game::print_chat("/survey is disabled");
  } else {
    Zeal::Game::print_chat("Usage: /survey on, /survey off");
    Zeal::Game::print_chat("Usage: /survey channel <response_channel_name>");
    Zeal::Game::print_chat("Usage: /survey new <question>");
    Zeal::Game::print_chat("Usage: /survey response <answer>");
    Zeal::Game::print_chat("Usage: /survey results (local chat), /survey share (to raid)");
  }
  return true;
}
